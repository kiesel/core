 ...
 */
class PharArchiveClassLoader extends AbstractClassLoader {
  ...
  public function __construct($archive) {
    $this->path= $archive instanceof PharArchive ? $archive->getURI() : $archive;

    // Archive within an archive
    if (0 === strncmp('phar://', $this->path, 7)) {
      $this->path= urlencode($this->path);
    }
    $this->archive= 'phar://'.$this->path.'/';
  }
  ...
  protected function classAtUri($uri) {
    ...
    // Absolute URIs have the form "xar://containing.phar/the/classes/Name.class.php"
    if ((DIRECTORY_SEPARATOR === $uri{0} || (':' === $uri{1} && '\\' === $uri{2}))) {
      return null;
    } else if (false !== ($p= strpos($uri, '.phar/'))) {
      $archive= substr($uri, 0, $p + 6);
  ...
  public function providesPackage($package) {
    $cmps= strtr($package, '.', '/').'/';
    $cmpl= strlen($cmps);
    return is_dir($this->archive.'/'.$cmps);
  }
   ...
   * @return  lang.archive.PharArchiveClassLoader
   */
  public static function instanceFor($path, $expand= true) {
    $path= $expand && 0 !== strncmp('phar://', urldecode($path), 6) ? realpath($path) : $path;
    if (!isset($pool[$path])) {
      self::$pool[$path]= new self($path);
    }
